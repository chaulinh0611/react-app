...
export const useCreateCard = () => {
    ...
    return useMutation({
        mutationFn: (payload: CreateCardPayload) => CardApi.createCard(payload).then((res) => res.data),
        onSuccess: () => {
...
export const useUpdateCard = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ id, payload }: { id: string; payload: UpdateCardPayload }) => CardApi.updateCard(id, payload).then((res) => res.data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["cards"] });
        },
    });
...
export const useReorderCard = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (payload: ReorderCardPayload) => CardApi.reorderCards(payload).then((res) => res.data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["cards"] });
        },
    });
